...
ROLES = [["Vert", "🟩", 1385358722645364736], ["Rouge", "🟥", 1385358689648771093]]


class RoleSelector(discord.ui.Select):
    def __init__(self):
        options = [
            discord.SelectOption(label=role[0], emoji=role[1], value=str(role[2])) for role in ROLES
        ]
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        if added_roles:
            await interaction.response.send_message(
                f":white_check_mark: Added role(s): {' '.join(added_roles)}", ephemeral=True
            )
...
class ResetButton(discord.ui.Button):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        await interaction.response.send_message(
            ":white_check_mark: **All roles where removed**", ephemeral=True
        )
